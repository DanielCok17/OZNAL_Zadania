...
```
1. riadok určuje prahovú hodnotu pre odstránenie stĺpcov s veľkým počtom chýbajúcich hodnôt (NA). Násobením 0.5 počtom riadkov (nrow(data)) nastavujeme prah, že akýkoľvek stĺpec s viac ako 50% chýbajúcich hodnôt bude odstránený z datasetu.
...
# Ensure predictions have the same factor levels
...
AUC hodnota je plocha pod ROC krivkou a poskytuje sumarizovaný ukazovateľ účinnosti modelu. AUC hodnota blízka k 1 značí vysokú prediktívnu schopnosť modelu, zatiaľ čo hodnota blízko k 0.5 naznačuje výkon, ktorý nie je lepší ako náhodné hádanie.
...
## Rozdiel medzi Klasifikaciou a SVM 

Rozdiel je v podstate vo výberu metódy. Klasifikácia je široký termín, ktorý zahŕňa akýkoľvek typ prediktívneho modelovania, ktoré rozdeľuje dáta do preddefinovaných tried alebo kategórií. SVM je špecifický typ klasifikačného modelu, ktorý sa snaží nájsť hyperrovinu (v prípade vysokorozmerných dát nazývanú aj hyperplocha) v priestore, ktorá najlepšie rozdelí dáta na požadované triedy.
...
Ak má Gradient Boosting najvyššiu AUC, znamená to, že je pravdepodobne najlepší z testovaných modelov.
...
1. model GLM:
Presnosť je porovnateľná s ostatnými dvoma modelmi, ale nie najlepšia.
Vyššia špecificita ako Gradient Boosting, ale nižšia ako SVM.
Stredná citlivosť a Kappa naznačujú priemerné výkony.

2. model SVM:
Tento model má vysokú špecificitu, čo znamená, že je veľmi dobrý v identifikácii negatívnych prípadov (defenzívni hráči).
Nízka citlivosť naznačuje, že model má problémy správne klasifikovať pozitívne prípady (ofenzívni hráči).
Relatívne nízka hodnota Kappa ukazuje na slabú dohodu nad rámec náhodnosti.

3. model Gradient Boosting
Vyššia presnosť v porovnaní s SVM.
Vysoká citlivosť značí, že model je efektívny v identifikovaní pozitívnych prípadov.
Nízka špecificita naznačuje, že model často nesprávne klasifikuje negatívne prípady ako pozitívne.
Vyššie hodnoty Kappa a Balanced Accuracy ako pri SVM, čo značí lepšie celkové výsledky.
...
Logistická regresia modeluje pravdepodobnosť výskytu udalosti využitím logistického rozdelenia.
Vypočíta sa pravdepodobnosť, že cieľová premenná patrí do jednej z kategórií (typicky 0 alebo 1) na základe lineárnej kombinácie vstupných premenných.
Výsledok je transformovaný pomocou logit funkcie (logistická funkcia), ktorá mapuje akúkoľvek reálnu hodnotu na interval (0,1), čo predstavuje pravdepodobnosť.

Výhody:

Poskytuje pravdepodobnostné výstupy pre predikcie.
Jednoduchá interpretácia výsledkov.
Robustná voči šumu a menej náchylná na overfitting pri malých dátových sadách.

Nevýhody:

Predpokladá lineárny vzťah medzi nezávislými premennými a logitom pravdepodobnosti.
Môže byť menej účinná, ak sú vzťahy medzi premennými nelineárne alebo ak existuje vysoká kolinearita.

## Support Vector Machine (SVM)

SVM hľadá hyperrovinu (alebo sady hyperrovín) v multidimenzionálnom priestore, ktorá najlepšie oddeluje rôzne triedy dát.
Optimalizuje okraj medzi najbližšími dátovými bodmi rôznych tried, známy ako podporné vektory.
Môže používať jadrové triky na efektívne mapovanie dát do vyšších rozmerov na nájdenie rozdelovacej hranice pre nelineárne klasifikačné problémy.

Výhody:

Efektívny v komplexných doménach, kde hranice medzi triedami nie sú jasne definované.
Funguje dobre na menších, čistých dátových sadách.
Flexibilný vo výbere jadier pre rôzne typy dát.

Nevýhody:

Môže byť náročný na výpočtové zdroje, najmä s veľkým počtom vstupných premenných alebo dátových bodov.
Náchylný na overfitting, najmä pri nedostatku regulácie alebo pri vysoko dimenzionálnych dátach.
...
Veľmi výkonný pre širokú škálu prediktívnych úloh.
Poskytuje vysokú presnosť a efektivitu.
Flexibilný vo výbere stratovej funkcie a podpory rôznych typov predikcií (binárne, multiklasové, regresné).

Nevýhody:

Často náchylný na overfitting, pokiaľ nie je správne nastavený (napr. hĺbka stromov, rýchlosť učenia).
Môže byť časovo náročný na trénovanie, najmä pri veľkých dátových sadách.

Rozdiely medzi 3 modelmi:

Zložitosť modelu: Logistická regresia je najjednoduchšia, zatiaľ čo SVM a Gradient Boosting sú zložitejšie a môžu modelovať zložitejšie vzťahy.

Predpoklady: Logistická regresia má striktnejšie predpoklady o dátach, SVM a Gradient Boosting sú flexibilnejšie voči nelinearitám.

Výkon: Gradient Boosting často dosahuje najlepší výkon na rôznych dátových sadách a problémoch, ale za cenu väčšej náchylnosti na overfitting a vyššej výpočtovej náročnosti. SVM je účinný v situáciách, kde je možné nájsť jasné rozdelenie tried, a logistická regresia je skvelá pre jednoduchšie problémy a rýchle prototypovanie.
...
Zaver:
Každý model má svoje silné a slabé stránky v závislosti od povahy údajov a toho, čo je potrebné predpovedať. Gradient Boosting poskytol najlepšiu celkovú presnosť a vyváženú presnosť, ale jeho nízka špecificita môže byť problém v určitých aplikáciách. Na druhej strane, SVM poskytol vynikajúcu špecificitu, ale na úkor citlivosťi, čo môže byť limitujúce, ak je dôležité identifikovať pozitívne prípady.
