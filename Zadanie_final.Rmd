...
# Pridanie nových príznakov a výber relevantných stĺpcov
data <- data %>%
  mutate(
    points = goals + assists,
    powerPlayPoints = power_play_goals + power_play_assists
  ) %>%
  select(goals, assists, points, penalty_minutes, plus_minus_ratings, powerPlayPoints, time_on_ice_per_game, role_oneHot)
```

# Hypotézy

-   **H0** : Počet gólov a asistencií/trestných minút/plus/minus hodnotení nemá statisticky významný vplyv na pravdepodobnosť byť klasifikovaný ako ofenzívny (alebo defenzívny pre H2) hráč.
-   **H1** : Počet gólov a asistencií/trestných minút/plus/minus hodnotení má statisticky významný vplyv na pravdepodobnosť byť klasifikovaný ako ofenzívny hráč.
...
model <- glm(role_oneHot ~ ., data = train_data, family = "binomial")  
```
...
Logistická regresia a Confusion Matrix: Presnosť modelu sa zlepšila na 0.7019, čo je lepšie v porovnaní s predchádzajúcim modelom.
To znamená, že približne 70% predikcií modelu je správnych.
Hodnota Kappa 0.3055 naznačuje umiernenú dohodu nad rámec náhody medzi predpovedanými a skutočnými klasifikáciami.
Senzitivita 0.8713 hovorí, že model je celkom dobrý v identifikovaní defenzívnych hráčov ako takých, ale špecifickosť 0.4107 ukazuje, že model má menšiu schopnosť správne klasifikovať ofenzívnych hráčov (teda dosť veľký podiel defenzívnych hráčov je nesprávne klasifikovaných ako ofenzívni).
...
Pravé negatívy (TN): 4514 - Počet prípadov, kde model správne predpovedal negatívnu triedu (napr. defenzívni hráči).
Falošné pozitívy (FP): 1776 - Počet prípadov, kde model nesprávne predpovedal pozitívnu triedu (napr. hráči, ktorí boli označení ako ofenzívni, ale v skutočnosti boli defenzívni).
Falošné negatívy (FN): 667 - Počet prípadov, kde model nesprávne predpovedal negatívnu triedu (napr. hráči, ktorí boli označení ako defenzívni, ale boli ofenzívni).
Pravé pozitívy (TP): 1238 - Počet prípadov, kde model správne predpovedal pozitívnu triedu (napr. ofenzívni hráči).
...
Proporcia pozitívnych predpovedí (ofenzívnych hráčov) je približne 23.25%.
Proporcia negatívnych predpovedí (defenzívnych hráčov) je približne 76.75%.
...
Zo všetkých prípadov, ktoré model predpovedal ako ofenzívne (ak sú ofenzívni hráči označení ako 'pozitívna' trieda), približne 65% z nich boli skutočne ofenzívni hráči.
...
V praxi to znamená, že keď sa model snaží identifikovať ofenzívnych hráčov (pokiaľ ofenzívni hráči sú označení ako 'pozitívna' trieda), správne ich odhalí s pravdepodobnosťou 41.1%.
...
To môže byť prekvapujúce a možno to vyžaduje ďalšiu analýzu kontextu alebo interakcií medzi príznakmi.

points: NA (nie je definované) z dôvodu singularity, t.j., points je dokonalou lineárnou kombináciou goals a assists, čo vedie k problémom s multicolinearitou.

penalty_minutes: Malý pozitívny koeficient, ale s p-hodnotou 0.31, naznačuje, že trestné minúty nemajú štatisticky významný vplyv na bytie ofenzívnym hráčom.

plus_minus_ratings: Negatívny koeficient ukazuje, že vyššie plus/minus hodnotenie znižuje log-odds byť ofenzívnym hráčom, čo môže naznačovať, že hráči s lepším hodnotením v tejto štatistike sú efektívnejší v defenzíve.

powerPlayPoints: Negatívny koeficient pre body získané počas presilovej hry tiež naznačuje, že ich zvýšenie vedie k zníženiu pravdepodobnosti byť klasifikovaným ako ofenzívny hráč.

time_on_ice_per_game: Záporný koeficient naznačuje, že s každou ďalšou sekundou strávenou na ľade sa pravdepodobnosť byť ofenzívnym hráčom znižuje.
To môže odzrkadľovať skutočnosť, že hráči s viac času na ľade môžu byť viac zameraní na defenzívne úlohy.

Pr(\>\|z\|): P-hodnoty pre goals, assists, plus_minus_ratings, powerPlayPoints a time_on_ice_per_game sú výrazne nižšie ako 0.05, čo naznačuje, že tieto príznaky majú štatisticky významný vplyv na pravdepodobnosť byť klasifikovaný ako ofenzívny hráč.

Naopak, penalty_minutes nemá štatisticky významný vplyv.

Null deviance vs. Residual deviance: Zníženie z null deviance na residual deviance ukazuje, že model s príznakmi výrazne lepšie predpovedá výsledok ako model bez príznakov (len s interceptom).

AIC (Akaike Information Criterion): Poskytuje meranie kvality modelu, kde nižšie hodnoty naznačujú lepší model vzhľadom na komplexnosť.
