...
```{r}
na_limits <- 0.5 * nrow(data)  # Limit pre vynechanie stĺpcov s nadmerným počtom chýbajúcich hodnôt
# Tento riadok určuje prahovú hodnotu pre odstránenie stĺpcov s veľkým počtom chýbajúcich hodnôt (NA). Násobením 0.5 počtom riadkov (nrow(data)) nastavujeme prah, že akýkoľvek stĺpec s viac ako 50% chýbajúcich hodnôt bude odstránený z datasetu.
data <- data[, colSums(is.na(data)) < na_limits]
# V tomto riadku sa odstraňujú stĺpce, ktoré prekračujú práve definovanú prahovú hodnotu chýbajúcich hodnôt. Funkcia colSums(is.na(data)) vypočíta počet chýbajúcich hodnôt (NA) v každom stĺpci, a ten sa potom porovná s hodnotou na_limits.
data <- data[, sapply(data, function(x) length(unique(x)) > 1)]  
# Tento riadok odstraňuje stĺpce s konštantnými alebo takmer konštantnými hodnotami. Používa sapply na aplikáciu funkcie na každý stĺpec, ktorá zistí, či počet unikátnych hodnôt v stĺpci je väčší ako 1. Ak nie, taký stĺpec nie je pre model užitočný a je odstránený.
data[] <- lapply(data, function(x) if(is.character(x)) factor(x) else x)  
# Tu prechádzame cez všetky stĺpce a konvertuje znakové stĺpce na faktory. Tento krok je dôležitý, pretože mnohé modelovacie techniky v R vyžadujú, aby kategorické premenné boli v tvare faktorov. Používa sa tu lapply, čo je vhodné, pretože vracia zoznam a priradenie data[] <- sa uistí, že sa zmeny aplikujú priamo na dataset data.
```

```{r}
# Vytvorenie tréningových a testovacích dat
training_indices <- createDataPartition(data$role_oneHot, p = 0.7, list = FALSE)
# Funkcia z knižnice caret je použitá na rozdelenie indexov dát do trénovacej a testovacej sady na základe cieľovej premennej data$role_oneHot.
train_data <- data[training_indices, ]
test_data <- data[-training_indices, ]
```
...
# spúšťa sa proces predikcie na testovacích dátach. Funkcia predict sa používa na vygenerovanie predpovedí s modelom SVM (svm_model), ktorý bol natrénovaný na trénovacej sade train_data.
...
```{r}
# Make sure svm_decision_values is a numeric vector
