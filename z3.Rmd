...
data %<>% mutate (
  shooter_type = case_when(
    ...
    goals >= 15 & goals < 30 ~ "average_shooter",
    goals >= 30 ~ "good_shooter"
  )
)
...
average_shooter_probability <- nrow(data %>% filter(shooter_type == "average_shooter")) / nrow(data)
good_shooter_probability <- nrow(data %>% filter(shooter_type == "good_shooter")) / nrow(data)

weak_shooter_probability
average_shooter_probability
...
data %<>% mutate (
  assist_type = case_when(
    assists >= 0 & assists < 15 ~ "weak_assister",
    assists >= 15 & assists < 30 ~ "average_assister",
    assists >= 30 ~ "good_assister"
  )
)
...
average_assister_probability <- nrow(data %>% filter(assist_type == "average_assister")) / nrow(data)
good_assister_probability <- nrow(data %>% filter(assist_type == "good_assister")) / nrow(data)

weak_assister_probability
average_assister_probability
...
middle_age_probability <- nrow(data %>% filter(age_type == "middle_age")) / nrow(data)
old_probability <- nrow(data %>% filter(age_type == "old")) / nrow(data)

young_probability
middle_age_probability
old_probability
```
...
data %<>% mutate (
  height_type = case_when(
    height >= 0 & height < 180 ~ "short",
    height >= 180 & height < 190 ~ "average_height",
    height >= 190 ~ "tall"
  )
)
...
average_height_probability <- nrow(data %>% filter(height_type == "average_height")) / nrow(data)
tall_probability <- nrow(data %>% filter(height_type == "tall")) / nrow(data)

short_probability
average_height_probability
tall_probability
```
...
data %<>% mutate (
  weight_type = case_when(
    weight >= 0 & weight < 80 ~ "light",
    weight >= 80 & weight < 100 ~ "average_weight",
    weight >= 100 ~ "heavy"
  )
)
...
average_weight_probability <- nrow(data %>% filter(weight_type == "average_weight")) / nrow(data)
heavy_probability <- nrow(data %>% filter(weight_type == "heavy")) / nrow(data)

light_probability
average_weight_probability
heavy_probability
```
...
data %<>% mutate (
  experience_type = case_when(
    ...
    experience >= 10 & experience < 20 ~ "veteran",
    experience >= 20 ~ "legend"
  )
)
...
veteran_probability <- nrow(data %>% filter(experience_type == "veteran")) / nrow(data)
